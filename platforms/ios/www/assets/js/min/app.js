 ...
 if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(mail))
  {
    ...
    myApp.alert("You have entered an invalid email address!","")
    return (false)
}
        ...
        function (status) {
            ...
            if (status.status == 'connected') {
                ...
                $.ajax({
                    ...
                    $('#userpicture').attr('src','https://graph.facebook.com/' + fb_id + '/picture?type=square');
...
function processFile(dataURL, fileType, filename) {
    ...
    image.onerror = function () {
        myApp.alert("There was an error processing your file!","");
    };
}
...
$('#registeruser1').on('click', function () {
            ...
            if (data == 1) {
              console.log("finish")
                   myApp.alert("Register successfully","Success!!",function() {
                               myApp.closeModal();
                               });
              mainView.router.load('index.html');
            } else {
                myApp.alert("This email have been used already","");
            }
            // console.log(length)
...
if ("list" == page.name) {
    ...
    if ("aboutclubaroy" == page.name) {
        $('#backabout').on('click', function() {
            mainView.router.back();
        });
    }
